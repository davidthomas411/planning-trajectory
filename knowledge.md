...
- Environment variable: `PLANEVAL_MONGODB_URI` (stored in `.env` locally).
- Code fallback: `DEFAULT_MONGODB_URI` in `src/data/trajectory_builder.py`.
- Recommendation: keep credentials only in `.env`; do not commit or paste credentials into logs/docs.

## ERD-Style Diagram (High-Level)

```
planeval
│
├─ evaluations (attempt-level DVH evaluations)
│   ├─ patient.patient_id
│   ├─ plan_id
│   ├─ attempt_number, created_at, approval.is_approved
│   ├─ standard_protocol / protocol.protocol_name
│   └─ results[] (structure, metric, goal/variation, achieved, priority)
│
├─ standard_protocols (canonical constraints per protocol)
│   ├─ protocol_name
│   └─ constraints[] (structure, objective, priority, goal/variation)
│
├─ protocols (template protocols, linked to standards)
│   ├─ protocol_name
│   ├─ constraints[] (structure, metric.display/objective, goal/variation)
│   └─ standard_ref {standard_id, standard_name, is_primary}
│
└─ custom_structure_aliases (structure name normalization)
    ├─ canonical
    └─ aliases[]
```
